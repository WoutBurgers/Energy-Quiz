...
public class ServerUtils {
    ...
    /**
     * Request for a search of activities by title.
     *
     * @param searchTerm the term that the activity title should contain
     * @return a list of searchResults
     * @throws IllegalArgumentException if there were too many results.
     */
    public List<SearchResult> search(String searchTerm) throws IllegalArgumentException {
        Response res = ClientBuilder.newClient(new ClientConfig()) //
                .target(server).path("api/question/search") //
                .queryParam("term", searchTerm)
                .request(APPLICATION_JSON) //
                .accept(APPLICATION_JSON) //
                .get();
        if (res.getStatus() == 204) {
            throw new IllegalArgumentException("Too many results from search: " +
                    "please choose a more specific search term.");
        }
        List<SearchResult> list = (List<SearchResult>) res.readEntity(new GenericType<>() { });
        return list;
    }

    /**
     * Gets a question from the server by id.
     * @param id the id of the requested question
     * @return the question
     * @throws IllegalArgumentException if the response has a status code other than 200
     *                                  (intended to be if there is no question with that id)
     */
    public Question getQuestion(long id) throws IllegalArgumentException {
        Response res;
        res = ClientBuilder.newClient(new ClientConfig()) //
                .target(server).path("api/question/" + id) //
                .request(APPLICATION_JSON) //
                .accept(APPLICATION_JSON) //
                .get();
    ...
    /**
     * Send a request to the server to generate a new set of questions.
     * Intended to be used after adding new activities.
     */
    public void generateQuestions() {
        ClientBuilder.newClient(new ClientConfig()) //
                .target(server).path("api/question/generate") //
                .request(APPLICATION_JSON) //
                .accept(APPLICATION_JSON) //
                .get();
    }
    ...
    /**
     * Sends a request for a string displaying all the questions.
     * @return A list of every question in the server.
     */
